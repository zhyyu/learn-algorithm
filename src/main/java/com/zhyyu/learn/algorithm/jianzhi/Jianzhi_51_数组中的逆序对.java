...
//        System.out.println(new Solution().reversePairs(new int[]{1, 3, 2, 3, 1}));
    ...
    static class Solution {
        ...
        public int reversePairs(int[] nums) {
            ...
            mergeSort(nums, fromIndex, (fromIndex + endIndex) / 2);
            mergeSort(nums, (fromIndex + endIndex) / 2 + 1, endIndex);
            merge(nums, fromIndex, (fromIndex + endIndex) / 2, endIndex);

            return reversePairsCnt;
        }

        private void mergeSort(int[] nums, int fromIndex, int endIndex) {
        ...
        private void merge(int[] nums, int fromIndex, int midIndex, int endIndex) {
            int[] temNums = Arrays.copyOf(nums, nums.length);

            int firstSegFromIndex = fromIndex;
            int firstSegEndIndex = midIndex;
            int secondSegFromIndex = midIndex + 1;
            int secondSegEndIndex = endIndex;

            for (int i = fromIndex; i <= endIndex; i++) {
                // 第一段 第二段均无值
                if (firstSegFromIndex > firstSegEndIndex && secondSegFromIndex > secondSegEndIndex) {
                    break;
                }

                // 第一段有值， 第二段无值
                if (firstSegFromIndex <= firstSegEndIndex && secondSegFromIndex > secondSegEndIndex) {
                    nums[i] = temNums[firstSegEndIndex];
                    firstSegFromIndex++;
                    continue;
                }
                ...
                // 第一段无值， 第二段有值
                if (firstSegFromIndex > firstSegEndIndex && secondSegFromIndex <= secondSegEndIndex) {
                    nums[i] = temNums[secondSegFromIndex];
                    secondSegFromIndex++;
                    continue;
                }
                ...
                // 第一段 第二段均有值
                if (firstSegFromIndex <= firstSegEndIndex && secondSegFromIndex <= secondSegEndIndex) {
                    if (temNums[firstSegFromIndex] < temNums[secondSegFromIndex]) {
                        nums[i] = temNums[firstSegFromIndex];
                        firstSegFromIndex++;
                        continue;
                    }
                    ...
                    if (temNums[firstSegFromIndex] >= temNums[secondSegFromIndex]) {
                        ...
                        reversePairsCnt = reversePairsCnt + (firstSegEndIndex - firstSegFromIndex + 1);
                        continue;
                    }
                }
